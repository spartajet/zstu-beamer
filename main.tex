...
\usetheme[logo=ZSTU, sublogo=AMSS]{zstu}
% logo 的选项: ZSTU
...
\title[浙江理工大学Beamer模板]{浙江理工大学 Beamer (\LaTeX{})}
\subtitle[非官方]{非官方的模版}
\author[郭晓忠]{\href{mailto:guoxiaozhong@zstu.edu.cn}{郭晓忠}}
\institute[ZSTU]{浙江理工大学 \\ 机械与自动控制学院}
\date[\today]{中国$\cdot$ 浙江 $\cdot$ 杭州 \\ \today}
\subject{展示主题}
\keywords{展示, 关键词}
...
\begin{frame}[t,fragile]
  ...
  \begin{CSharpCode}
    for (int i = 0; i < this.RowCount; i++)
    {
        ...
        {
            Mat tempMat = Cv2.ImRead(this.FileNames[i], ImreadModes.AnyDepth);
            if (tempMat.Type() != MatType.CV_16UC1)
            {
                MessageBox.Show(this, "图像尺寸不正确", "提示", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            Mat rect = result[new OpenCvSharp.Rect(width * j, height*i, width, height)];
            tempMat.CopyTo(rect);
        }
\end{CSharpCode}
...
\begin{frame}
  ...
  \inputminted[firstline=2,lastline=15]{csharp}{E://CSharp//ImageWiter//ImageWiter//MainWindow.xaml.cs}
